...
class ValidationTests extends FunSuite with DataFrameSuiteBase {
  ...
  ignore( "Validation: When input contains no null values for risk_reference validation script should return count 0" ) {
    ...
    val statement = utils.loadHQLStatementFromResource( "Validation/ndex/Policy/CheckForNullRiskReference.hql" )
  ...
  ignore( "Validation: When input contains rows with risk_reference null validation script should return count 1" ) {
