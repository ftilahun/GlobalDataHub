...
SELECT
	...
	CONCAT(ZUMADF00.COMANU, ZUMADF00.COMASE, ZUCODF00.CORKRS, ZUCODF00.COSDSQ, ZUCODF00.COCVSQ) AS coveragereference,
    false AS iscashtransactiontype,
    IF(ICDCREP.IAVYPG = 'PERCNT' AND ZUMADF00.MAPORF NOT LIKE 'X%' AND ZUMADF00.MAPORF NOT LIKE '9%',
    	ROUND(CAST(IFNULL(ZUCEDF00.CECVA1, 0) AS DECIMAL(15, 2)) * CAST(IFNULL(ZUSPDF00.SPSPSP, 0) AS DECIMAL(11, 7)) / 100.00 * CAST(IFNULL(ZUDDDF00.DDDDPC, 0) AS DECIMAL(11, 7)) / 100.00 , 2)) AS originalamount,
    IF(ICDCREP.IAVYPG = 'FLAT' AND ZUMADF00.MAPORF NOT LIKE 'X%' AND ZUMADF00.MAPORF NOT LIKE '9%',

    		)
    	 AS originalamount,
    ZUCEDF00.CEOGCU AS originalcurrencycode,
	ZUMADF00.MAPORF AS policynumber,
	CONCAT(ZUMADF00.COMANU, ZUMADF00.COMASE, ZUCODF00.CORKRS, ZUCODF00.COSDSQ) AS sectionreference,
	IF(ICDCREP.IAVYPG = 'PERCNT' AND ZUMADF00.MAPORF NOT LIKE 'X%' and ZUMADF00.MAPORF NOT LIKE '9%',
		ROUND(CAST(IFNULL(ZUCEDF00.CECVA1 / ZUCEDF00.CEEAEA, 0) AS DECIMAL(15, 2)) * CAST(IFNULL(ZUSPDF00.SPSPSP, 0) AS DECIMAL(11, 7)) / 100.00 * CAST(IFNULL(ZUDDDF00.DDDDPC, 0) AS DECIMAL(11, 7)) / 100.00, 2) AS settlementamount,
...
LEFT JOIN ZUSKDF00 ON ZUSKDF00SKMANU = ZUSFDF00SFMANU
