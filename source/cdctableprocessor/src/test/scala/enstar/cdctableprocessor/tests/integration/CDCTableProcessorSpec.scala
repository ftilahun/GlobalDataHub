...
import enstar.cdctableprocessor.io.{ AvroDataFrameReader, AvroDataFrameWriter, CDCDataFrameReader, CDCDataFrameWriter }
 ...
 */
class CDCTableProcessorSpec extends FlatSpec
    with GivenWhenThen
    with Matchers {
  ...
  "CDCTableProcessor" should "Process a table" in {
    ...
    val reader = new CDCDataFrameReader(
    ...
    baseRecords.collect().zipWithIndex.foreach { rowWithInt =>
      ...
      val validFrom = DateTimeFormat.forPattern("YYYY-MM-dd HH:mm:ss.SSS").parseDateTime(row.getString(2))
      val validTo = DateTimeFormat.forPattern("YYYY-MM-dd HH:mm:ss.SSS").parseDateTime(row.getString(3))
    ...
    changeRecords.collect().zipWithIndex.foreach{ rowWithInt =>
      ...
      val validTo = DateTimeFormat.forPattern("YYYY-MM-dd HH:mm:ss.SSS").parseDateTime(row.getString(6))
      ...
      val validFrom = DateTimeFormat.forPattern("YYYY-MM-dd HH:mm:ss.SSS").parseDateTime(row.getString(7))
    ...
    unprocessed.collect().foreach{ row =>
      ...
      formatter.parseDateTime(row.getString(6)).isAfter(formatter.parseDateTime(attunityCutoff)) should be (true)
    }
