...
class CDCTableProcessorSpec extends FlatSpec with GivenWhenThen with Matchers {
  ...
  "CDCTableProcessor" should "load the initial table data" in {
    ...
    Mockito
      .when( properties.getStringProperty( "dataBasePath" ) )
      .thenReturn( "/path/to/initialdata" )
    Mockito
      .when( properties.getStringProperty( "timestampColumnName" ) )
    ...
    Mockito
      .when( properties.getStringProperty( "sequenceNumberColumnName" ) )
      .thenReturn( "lastchangesequence" )
    Mockito
      .when( properties.getStringProperty( "isDeletedColumnName" ) )
      .thenReturn( "isdeleted" )
    Mockito
      .when( properties.getStringProperty( "changeOperationColumnName" ) )
      .thenReturn( "_operation" )
    Mockito.when(properties.getBooleanProperty("filteronchangemask")).
      thenReturn(false)
    Mockito
      .when(
  ...
  "CDCTableProcessor" should "load the change table data" in {
    ...
    cdcTableProcessor.load( tableName,
      TestContexts.sqlContext,
      controlProcessor,
      reader,
      userFunctions,
  ...
  "CDCTableProcessor" should "Register a temp table and query the data" in {
    ...
    Mockito.when(controlProcessor.getLastSequenceNumber(
      org.mockito.Matchers.any( classOf[SQLContext] ),
      org.mockito.Matchers.any( classOf[SQLFileReader] ),
      org.mockito.Matchers.any( classOf[GDHProperties] ),
      org.mockito.Matchers.anyString()
    )).thenReturn(changeSeq)
    Mockito.when(tableOperations.registerTempTable(
      org.mockito.Matchers.any(classOf[DataFrame]),
      org.mockito.Matchers.anyString()
    )).thenCallRealMethod()
    Mockito.when(tableOperations.deRegisterTempTable(
      org.mockito.Matchers.any(classOf[SQLContext]),
      org.mockito.Matchers.anyString()
    )).thenCallRealMethod()
    Mockito
    ...
    val data = cdcTableProcessor.process(tableName,TestContexts.sqlContext,controlProcessor,
      properties,reader,userFunctions,tableOperations,sqlReader)
  ...
  "CDCTableProcessor" should "Filter columns on a change mask" in {
    ...
    Mockito.when(properties.getBooleanProperty("filteronchangemask")).
      thenReturn(true)
    Mockito.when(properties.getStringProperty("changeMaskColumnName")).
      thenReturn("_changemask")
    Mockito.when(properties.getArrayProperty("columnPositions" + tableName)).
    ...
    Mockito.when(userFunctions.isBitSet(org.mockito.Matchers.anyString(),
      org.mockito.Matchers.anyInt())).
      thenCallRealMethod()
    Mockito.when(userFunctions.getBitMask(org.mockito.Matchers.anyString())).
      thenCallRealMethod()
    Mockito.when(userFunctions.isAnyBitSet(
      org.mockito.Matchers.anyString(),
