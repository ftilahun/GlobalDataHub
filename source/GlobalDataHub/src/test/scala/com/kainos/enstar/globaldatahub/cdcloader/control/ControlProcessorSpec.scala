...
class ControlProcessorSpec extends FlatSpec
  ...
  "Controlprocessor" should "Register the control table" in {
    ...
    Mockito.when( properties.getStringProperty("controlTablePath") ).thenReturn( "/control/dir/" )
    Mockito.when( properties.getStringProperty("controlTableName") ).thenReturn( "control" )
    When( "The input is valid" )
    Mockito.when( loaderIO.read( TestContexts.sqlContext, properties.getStringProperty("controlTablePath"), None ) )
    ...
    TestContexts.sqlContext.sql( "SELECT * FROM " + properties.getStringProperty("controlTableName") ).count should be ( 10 )

    Given( "The control table has been created" )
    When( "The control table is de-registered" )
    Mockito.when( loaderIO.deRegisterTempTable( TestContexts.sqlContext, properties.getStringProperty("controlTableName") ) ).
      thenCallRealMethod()
    controlProcessor.deregisterControlTable
    Then( "an exception should be thrown" )
    an[RuntimeException] should be thrownBy {
      TestContexts.sqlContext.sql( "SELECT * FROM " + properties.getStringProperty("controlTableName") )
    }
  }
  ...
  "Controlprocessor" should "Retrieve the last sequence processed" in {
    ...
    Mockito.when( properties.getStringProperty("controlTablePath")).thenReturn( "/control/dir/" )
    Mockito.when( properties.getStringProperty("controlTableName") ).thenReturn( "control" )
    Mockito.when(properties.getStringProperty("changeSequenceTimestampFormat")).thenReturn("YYYYMMDDHHmmSShh")
    Mockito.when( loaderIO.read( TestContexts.sqlContext, properties.getStringProperty("controlTablePath"), None ) )
      .thenReturn( TestContexts.generateControlTable( 10 ) )
    Mockito.when( loaderIO.registerTempTable( org.mockito.Matchers.any( classOf[DataFrame] ),
      org.mockito.Matchers.anyString() ) ).thenCallRealMethod()
    ...
    Mockito.when(properties.getStringProperty("controlTableSQLPath")).thenReturn("/some/path")
    Mockito.when(sqlReaderIO.getSQLString( TestContexts.sparkContext, properties.getStringProperty("controlTableSQLPath") ))
    ...
    controlProcessor.getLastSequenceNumber("'policy'") should be (
      DateTimeFormat.forPattern( properties.getStringProperty("changeSequenceTimestampFormat") ).print( date ) +
      "0000000000000000010"
    )
