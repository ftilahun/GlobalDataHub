 ...
 */
class CDCLoaderIOSpec extends FlatSpec with GivenWhenThen with Matchers {
  ...
  "CDCLoaderIO" should "Read input data" in {
    ...
    Mockito
      .when( dataframeIO.read( TestContexts.sqlContext, "/some/path/", None ) )
    ...
    Mockito
      .when(
        dataframeIO.read( TestContexts.sqlContext, "/some/invalid/path/", None ) )
      .thenThrow( classOf[InvalidInputException] )
    When( "The path does not exist" )
    Then( "An exception should be raised" )
    an[PathNotFoundException] should be thrownBy {
      cdcLoaderIO.read( TestContexts.sqlContext, "/some/invalid/path/", None )
    }
  }
  ...
  "CDCLoaderIO" should "Write output data" in {
    ...
    Mockito
      .when(
        dataframeIO.write( TestContexts.sqlContext,
          "/some/path",
          data,
          Some( StorageLevel.MEMORY_ONLY ) ) )
      .thenReturn( true )
    val outCount = cdcLoaderIO.write( TestContexts.sqlContext,
      "/some/path/",
      data,
    ...
    an[PathExistsException] should be thrownBy {
      cdcLoaderIO.write( TestContexts.sqlContext,
        "/some/existing/path",
        data,
        StorageLevel.MEMORY_ONLY )
    }
