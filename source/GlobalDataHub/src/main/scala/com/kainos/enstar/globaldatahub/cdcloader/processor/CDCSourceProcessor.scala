...
class CDCSourceProcessor extends Logging {
  ...
  def process(controlProcessor : ControlProcessor,
    ...
    properties.getArrayProperty( "spark.cdcloader.input.tablenames" ).foreach {
      tableName =>
      //process each table
      val tableData = tableProcessor.process(tableName,sqlContext,controlProcessor,properties,reader,
        userFunctions,tableOperations,sqlReader)
      //save each table
      tableProcessor.save(sqlContext,writer,properties,tableData,tableName)
    }
