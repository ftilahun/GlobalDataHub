...
class CDCTableProcessor extends TableProcessor {
  ...
  def process(tableName: String,
    ...
    val sqlString = sqlReader.getSQLString(
      ...
      properties.getStringProperty("sqlBasePath") +
        tableName)
  ...
  def save(sqlContext: SQLContext,
    ...
    writer.write(sqlContext,
                 properties.getStringProperty("dataSavePath") + tableName,
                 dataFrame,
  ...
  def load(tableName: String,
    ...
    val isAnyBitSet = udf(
      userFunctions.isAnyBitSet(
        _: String,
        properties.getArrayProperty("columnPositions" + tableName)))
    ...
    val output =
      if (controlProcessor.isInitialLoad(sqlContext, tableName, properties)) {
        ...
        reader
          .read(sqlContext,
                properties.getStringProperty("dataBasePath") + tableName,
                //storage level will require tuning based on performance.
                Some(StorageLevel.MEMORY_AND_DISK_SER))
          .withColumn(properties.getStringProperty("sequenceNumberColumnName"),
                      generateSequenceNumber())
          .withColumn(
            properties.getStringProperty("changeOperationColumnName"),
            operation())
      } else {
        //load change data
        val data = reader.read(
          sqlContext,
          properties.getStringProperty("dataBasePath") + tableName +
            properties.getStringProperty("changeTableSuffix"),
          //storage level will require tuning based on performance.
          Some(StorageLevel.MEMORY_AND_DISK_SER))

        if (properties.getBooleanProperty("filteronchangemask")) {
          data.filter(isAnyBitSet(
            data(properties.getStringProperty("changeMaskColumnName"))))
        } else {
          data
        }
      }
    ...
    output
      .withColumn(properties.getStringProperty("timestampColumnName"),
                  timeStamp())
      .withColumn(
        properties.getStringProperty("isDeletedColumnName"),
        isDeleted(
          output(properties.getStringProperty("changeOperationColumnName"))))
  }
